...
test('锚点守卫：getTotalDays() 在锚点当天为 0、后一天 +1、前一天 -1', function (t) {
  pg.PAGES.forEach(function (meta) {
    t.test(meta.name, function () {
...
test('周期性不变量：班次(d) === 班次(d + N)，锚点前后各 60 天', function (t) {
  pg.PAGES.forEach(function (meta) {
    const probe = mp.load(meta.rel, pg.extraDataFor(meta))
    const setters = pg.banzuSetters(probe)
    const groups = setters.length ? setters : [null]
    groups.forEach(function (setter) {
      const label = meta.name + (setter ? ' ' + setter : '')
      t.test(label, function () {
        ...
        for (let off = -60; off + meta.period <= 60; off++) {
          assert.strictEqual(memo[off], memo[off + meta.period],
            label + ' 偏移 ' + off + ' 与偏移 ' + (off + meta.period) + ' 应同班：「' +
...
test('经警队班表（与真实排班核对过）：上一天休三天，4 个班循环', function (t) {
  ...
  t.test('个人页：锚点前后各 200 天，每天恰好 1 个班上班，且与总览页一致', function () {
  ...
  t.test('个人页切到一班后，7 天列表是「上1休3」', function () {
    const worker = mp.load('pages/jjd/worker/worker.js')
...
test('金标准快照：锁定 2026-09-11 起 7 天的 21 组已校准输出', function (t) {
  const current = {}
  pg.PAGES.forEach(function (meta) {
    const probe = mp.load(meta.rel, pg.extraDataFor(meta))
    const setters = pg.banzuSetters(probe)
    ...
    groups.forEach(function (setter) {
      ...
      current[meta.rel + (setter ? '#' + setter : '')] = pg.rows(page)
    })
  })
  ...
  if (UPDATE_GOLDEN) {
    ...
    t.skip('GOLDEN_UPDATE=1：已重写 ' + path.relative(mp.REPO, GOLDEN_FILE))
    return
  }
  ...
  Object.keys(current).sort().forEach(function (key) {
    t.test(key, function () {
      assert.deepStrictEqual(current[key], golden.rows[key],
        '班次输出与金标准不一致。若确实重新校准过班表，用 GOLDEN_UPDATE=1 重跑本测试更新基线')
    })
  })
})
...
test('日历格子的班次与列表模式同一天完全一致（不允许有第二套算法）', function (t) {
    ...
    groups.forEach(function (setter) {
      ...
      t.test(label, function () {
        ...
        MONTHS.forEach(function (ym) {
          ...
          assert.ok(cal, label + ' ' + Y + '-' + (M + 1) + ' 的日历数据不该为空')
          ...
          cal.weeks.forEach(function (w) {
            ...
            w.cells.forEach(function (c) {
              ...
              assert.strictEqual(c.text, calendarTextOf(meta, truth[c.day]),
                label + ' ' + dayLabel(Y, M, c.day) + ' 日历「' + c.text +
                '」与列表「' + truth[c.day] + '」不一致')
...
test('9 个倒班页都接好了月日历', function (t) {
  pg.PAGES.forEach(function (meta) {
    t.test(meta.name, function () {
      ...
      assert.strictEqual(pg.stripDate(page.data.r1), pg.stripDate(
        (function () {
          const p2 = mp.load(meta.rel, pg.extraDataFor(meta))
          const s = pg.banzuSetters(p2)[0]
          if (s && page.data.banzuId) p2['setBanzu' + page.data.banzuId]()
          atDate(p2, 2026, 8, 20)
          return p2.data.r1
        })()
      ), '点日历某天后 r1 应是那一天的班次')
