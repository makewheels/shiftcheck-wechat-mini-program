...
- [x] 上传开发版本（2026-09-14，2.4.0，代码包 260.9 KB）
- [ ] 后台把 2.4.0 设为**体验版** → 按检查单第 9 节真机走一遍 → 提审 → 发布
...
- [ ] **README 的 2.4.0 日志里有两条「修了又被删」**：「修复『自定义规则提醒』页打不开」和
      「修复 pushHome 把位运算 `&` 当逻辑与 `&&` 用」—— 这两个页面在**同一版**里随链路一起删了，
      读起来自相矛盾。两件事都真实发生过，所以按原样保留（Release 说明里加了一句注解）。
      要清理的话，删掉这两条、或改写成「修复后该页在本版随链路删除」都行
